...
class GscIndexInspect:
    def __init__(self, sheets_url, token):
        self.token = token
        # save sheets url to db
        InspectSheetsURL(inspect_cred=CredentialsModel.objects.get(token=token),
                         sheets_url=sheets_url).save()
    ...
    def authorization_gsc(self):
        # saving credential to db it's async so we need wait and check whether the data has already been saved
        while True:
            if CredentialsModel.objects.get(token=self.token).credential:
                break
            else:
                sleep(1)
    ...
    def get_url_from_sheets_and_save_to_sheets(self):
        ...
        row_num = 2
        for url in self.colA[1:]:
            # sometimes someone add url without start as http
            ...
            try:
                user_canonical = str(response['inspectionResult']['indexStatusResult']['userCanonical'])
            except KeyError:
